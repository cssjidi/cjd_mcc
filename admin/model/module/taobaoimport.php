...
class ModelModuleTaobaoImport extends Model {
...
//    fclose($handle);
		...
		if ($bom === chr(0xff) . chr(0xfe) || $bom === chr(0xfe) . chr(0xff)) {
			...
			$encoding = mb_detect_encoding($file_sample, 'UTF-8,GBK,GB2312,UNICODE,BIG5');
		}
		if ($encoding) {
	...
	{
		if ($this->fopen_utf8($reader, "r") === FALSE){
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get( 'error_type' )."\n",3,DIR_LOGS."error.txt");
			return FALSE;
		}
	...
	function upload( $file ) {
		if (($handle = $this->fopen_utf8($file['tmp_name'], "r")) === FALSE) return;
		$row = 1;
		$array=array();
