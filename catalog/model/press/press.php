	...
	public function getPresses($data = array()) {
		...
		if (!$press_data) {
			...
			foreach ($query->rows as $result) {
				...
				$press_data[$result['press_id']]['categories'] = $this->getPressCategory($result['press_id']);

				///foreach ($press_categories as $press_category)
				//array_push($press_data[$result['press_id']],$this->getPressCategory($result['press_id']));
//				echo '<pre>';
//				print_r($press_data);
//				echo '</pre>';
			}
	...
	public function getPressCategory($press_id){
		$sql = "SELECT DISTINCT * FROM " . DB_PREFIX . "press_category_description mpcd LEFT JOIN " . DB_PREFIX . "press_category mpc ON (mpcd.press_category_id=mpc.press_category_id) LEFT JOIN " . DB_PREFIX . "press_to_press_category mptc ON(mpc.press_category_id = mptc.press_category_id) LEFT JOIN " . DB_PREFIX . "press_description mpd ON(mpd.press_id = mptc.press_id) LEFT JOIN " . DB_PREFIX . "press mp ON(mp.press_id = mpd.press_id) WHERE mp.press_id='". $press_id ."' AND mpcd.language_id='" . (int)$this->config->get('config_language_id') . "' AND mpd.language_id='" . (int)$this->config->get('config_language_id') . "'";
		$query = $this->db->query($sql);
		if ($query->num_rows) {
			$press_data = array(
				'press_id'       => $query->row['press_id'],
