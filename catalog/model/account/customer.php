...
class ModelAccountCustomer extends Model {
	public function addCustomer($data, $weixin_login_openid=0, $weixin_login_unionid=0,$qq_openid=0) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', fullname = '" . $this->db->escape($data['fullname']) . "', email = '" . $this->db->escape($data['email']) . "', weixin_login_unionid = '" . $this->db->escape($weixin_login_unionid) . "', weixin_login_openid = '" . $this->db->escape($weixin_login_openid) ."', qq_openid = '" . $this->db->escape($qq_openid) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW()");
	...
	public function getTotalCustomerByqqOpenid($openid) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE qq_openid = '" . $this->db->escape($openid) . "'");

		return $query->row['total'];
	}

	public function getCustomerByqqOpenid($openid) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE qq_openid = '" . $this->db->escape($openid) . "'");

		return $query->row;
	}

	public function editCustomerByqqOpenid($data,$qq_openid) {
		$customer_id = $this->customer->getId();

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET fullname = '" . $this->db->escape($data['fullname']) . "', qq_openid = '" . $this->db->escape($qq_openid) . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
